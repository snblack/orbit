# Phase 1: Core — Недели 1–4

Цель: рабочий MVP с онбордингом, матчингом, Pod-дашбордом и реалтайм-чатом.

---

## 1.1 Проект и инфраструктура

- [x] Создать Rails-приложение (`rails new orbit --database=postgresql`)
- [x] Установить и настроить Tailwind CSS (`tailwindcss-rails`)
- [x] Установить Hotwire (`hotwire-rails` или встроен в Rails 7)
- [x] Настроить Devise (регистрация, логин, подтверждение email)
- [x] Подключить фоновые задачи (Solid Queue, встроен в Rails 8 — Redis не нужен)
- [x] Настроить pgvector расширение в PostgreSQL (`CREATE EXTENSION vector`) ⏳ отложено до фазы AI-матчинга
- [x] Установить `neighbor` gem для vector search ⏳ отложено до фазы AI-матчинга
- [x] Настроить ActiveStorage + Cloudflare R2 (для фото профиля) ⏳ отложено до фазы профиля
- [x] Базовый layout с Tailwind: навигация, мобильный responsive

---

## 1.2 Онбординг-квиз

**Цель:** пользователь проходит структурированный квиз и данные сохраняются в профиль.

- [x] Модель `User` расширить полями профиля:
  - `life_phase` (enum: recently_moved, career_change, new_parent, remote_worker, etc.)
  - `social_style` (enum: introvert, ambivert, extrovert)
  - `friendship_goal` (enum: support, growth, fun, intellectual)
  - `schedule_preference` (jsonb: дни недели + время)
  - `social_frequency` (enum: once_week, two_three_week, daily)
  - `openness_level` (enum: surface, moderate, deep)
  - `interests` (array: string[])
  - `location_district` (string)
  - `latitude`, `longitude` (float)
  - `onboarding_completed` (boolean)
- [x] Multi-step форма онбординга (5–6 шагов) с Turbo Frames
- [x] Шаг 1: Жизненная фаза (radio cards)
- [x] Шаг 2: Ценности и мироощущение (radio + checkboxes)
- [x] Шаг 3: Ритм жизни (расписание, частота встреч)
- [x] Шаг 4: Интересы (checkbox grid + открытое поле)
- [x] Шаг 5: Геолокация (автоопределение браузером или ручной ввод района)
- [x] Шаг 6: Фото + короткое видео-представление (15 сек, опционально) ⏳ загрузка отложена до ActiveStorage
- [x] Progress bar через весь квиз
- [x] Валидации на каждом шаге, сохранение через `PATCH /onboarding/:step`

---

## 1.3 Алгоритм матчинга (Phase 1: взвешенный)

**Цель:** собрать Pod из 5 человек без AI, на чистой Ruby-логике.

- [x] Модели: `Pod`, `PodMembership`
- [x] `PodMatcher` сервис (`app/services/pod_matcher.rb`):
  - Фильтр по городу/району (Geocoder, радиус ≤ 30 мин)
  - Пересечение окон свободного времени
  - Скоринг совместимости:
    - Жизненная фаза (вес 30%)
    - Тип общения intro/extro (вес 20%)
    - Ценностное сходство (вес 25%)
    - Частота встреч (вес 25%)
  - Группировка в Pod по 5 человек
- [x] Admin-action или Rake-задача для запуска матчинга вручную
- [x] После матчинга: email пользователям "Вы попали в Pod"
- [x] `Connector`-роль: автоматически назначается участнику с наибольшим extroversion

---

## 1.4 Pod Dashboard

**Цель:** главный экран пользователя — всё про его Pod.

- [x] `PodController#show` — главная страница
- [x] Список участников Pod: аватар, имя, краткое bio
- [x] Текущая фаза Pod (1–12 неделя) с описанием что происходит
- [x] "Last seen together": последняя совместная активность с датой
- [x] Ближайшая запланированная активность (если есть)
- [x] Кнопка "Предложить активность"
- [x] Turbo Frame: обновление без перезагрузки при действиях
- [x] Страница профиля участника Pod (только для Pod-членов)

---

## 1.5 Realtime чат

**Цель:** групповой чат внутри Pod через ActionCable.

- [x] Модель `Message` (pod_id, user_id, body, created_at)
- [x] `PodChannel` — ActionCable channel
- [x] UI чата: список сообщений + поле ввода
- [x] Turbo Streams broadcast при новом сообщении
- [x] Индикатор "онлайн" участников Pod (presence через ActionCable)
- [x] Автопрокрутка вниз при новых сообщениях (Stimulus controller)
- [x] Пагинация старых сообщений (load more)
- [x] Карточки вопросов в чате (статичные 36 Questions на Phase 1)
